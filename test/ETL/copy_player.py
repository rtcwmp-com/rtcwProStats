...
logger = logging.getLogger('copy_player')
logger.setLevel(log_level)

log_stream_name = "local"


def copy_player(ddb_table, old_guid, new_guid):
    """
    When player changed a guid, but would want to retain elos, agg_stats, and achievements,
    this code will copy their OLD stuff under player#old_guid into player#new_guid
    LAST FEW items under new_guid WILL BE LOST
    """
    old_pk = "player#" + old_guid
    new_pk = "player#" + new_guid
    old_response = ddb_table.query(KeyConditionExpression=Key('pk').eq(old_pk))

    new_items = []
    for ddb_item in old_response["Items"]:
        ddb_item["pk"] = new_pk
        new_items.append(ddb_item)
        if "updated" in ddb_item:
            backdate = '1999' + ddb_item["updated"][4:]
            logger.info("Adjusting old player date values")
            ddb_table.update_item(Key={'pk': old_pk, 'sk': ddb_item["sk"]},
                                  UpdateExpression='SET updated = :backdate',
                                  ExpressionAttributeValues={':backdate': backdate}
                                  )
    logger.info("Copying over old player to new player with this many items: " + str(len(new_items)))
    ddb_batch_write(ddb_client, ddb_table.name, new_items)
...
if __name__ == "__main__":
    old_guid = ''
    new_guid = ''
    copy_player(ddb_table, old_guid, new_guid)
    # move_elos(ddb_table)
