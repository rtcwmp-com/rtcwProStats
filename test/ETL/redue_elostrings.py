...
def fix_elos(ddb_table, region, type_):
    pk_base = "leaderelo#"
    ...
    if response['Count'] > 0:
        for record in response["Items"]:
            ...
            elo_str = record['gsi1sk']
            elo_int = int(record['gsi1sk'])

            if len(elo_str) == 3 and elo_int < 1000:
                new_elo = int(elo_int * 4.454)
                if new_elo < 1200:
                    new_elo = 1200
                print("Adjust: " + record.get("real_name", "n/a") + " " + str(new_elo))
                new_record = record.copy()
                new_record["gsi1sk"] = str(new_elo)
                new_record["data"] = str(new_elo)
                new_items.append(new_record)
            else:
                print("Skip  : " + record.get("real_name", "n/a") + " " + elo_str)

        logger.info("Fixing elos")
        # print(new_items)
...
if __name__ == "__main__":
    ...
    types = ["6", "3", "6plus", "unk"]
    fix_elos(ddb_table, regions[3], types[3])
    # move_elos(ddb_table)
